...
def generate_models(data: bytes, limit: int):
    ...
    def all():
        for _, m, _ in s.get_models(limit):
            yield eval(repr(list(x for c in s.courses for x in concretize(c.evaluate(m)))))
    return json.dumps({'schedules': list(all())}, separators=(',', ':'))
...
if __name__ == '__main__':
    ...
    for i, m, s in sched.get_models(limit):
        ...
        data = list(x for c in sched.courses for x in concretize(c.evaluate(m)))
        out = eval(repr(data))
