...
class ConcurrentClient:
    ...
    def create_session(self) -> bool:
        ...
        try:
            ...
            # Randomly choose different optimizer options for variety
            all_options = [
                "faculty_course",
                "faculty_room",
                "faculty_lab",
            ...
            modified_config["optimizer_options"] = random.sample(
                all_options, random.randint(3, len(all_options))
            )

            response = self.session.post(
                f"{BASE_URL}/submit", json=modified_config, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Client {self.client_id}: Failed to create session: {response.status_code}"
                )
                return False
    ...
    def get_session_details(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(
                f"{BASE_URL}/schedules/{self.schedule_id}/details", timeout=5
            )
            if response.status_code == 200:
                return response.json()
            else:
                print(
                    f"Client {self.client_id}: Failed to get details: {response.status_code}"
                )
                return {}
        except Exception as e:
    ...
    def generate_schedule(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{BASE_URL}/schedules/{self.schedule_id}/next", timeout=60
            )

            if response.status_code == 200:
                result = response.json()
                print(
                    f"Client {self.client_id}: Generated schedule {result['index'] + 1}"
                )
                return result
                ...
                print(
                    f"Client {self.client_id}: Failed to generate schedule: {response.status_code}"
                )
                return {}
    ...
    def get_schedule_by_index(self, index: int) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(
                f"{BASE_URL}/schedules/{self.schedule_id}/index/{index}", timeout=5
            )
            if response.status_code == 200:
                return response.json()
            else:
                print(
                    f"Client {self.client_id}: Failed to get schedule {index}: {response.status_code}"
                )
                return {}
        except Exception as e:
    ...
    def cleanup(self) -> bool:
        ...
        try:
            response = self.session.delete(
                f"{BASE_URL}/schedules/{self.schedule_id}/delete", timeout=5
            )
            if response.status_code == 200:
                ...
                print(
                    f"Client {self.client_id}: Failed to delete session: {response.status_code}"
                )
                return False
        except Exception as e:
...
def client_workflow(client_id: int, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if details:
            print(
                f"Client {client_id}: Session limit={details.get('limit')}, optimizer_options={details.get('optimizer_options')}"
            )
        ...
        print(
            f"Client {client_id}: Completed successfully - {results['schedules_generated']} schedules generated"
        )
...
def test_health_check() -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"✓ Server is healthy. Active sessions: {health_data['active_sessions']}"
            )
            return True
        else:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=NUM_CLIENTS) as executor:
        ...
        future_to_client = {
            executor.submit(client_workflow, i, config): i for i in range(NUM_CLIENTS)
        }
