...
Global resources are `RoomConfig(name=..., capacity=..., features=..., times=...)` and
`LabConfig(name=..., capacity=..., features=..., times=...)` objects; course candidate lists and faculty preference
keys continue to use resource-name strings. Every `CourseConfig` requires a positive expected-enrollment `capacity`
and can additionally declare `section_id`, `modality`, required resource features, and
...
`audit_schedule()` does not query Z3. The independent auditor validates the supplied decoded schedule against
normalized policies and reports workload, resource use, enabled objective scores, and unmet preference outcomes.
Unknown or ineligible assignment values are returned as violations rather than causing lookup failures.
...
`CandidateDomainDiagnostic`, `CapacityDiagnostic`, `DayFeasibilityDiagnostic`,
`ProvenanceEdge`, `RelaxationSuggestion`, `ScheduleAudit`, `FacultyWorkloadDiagnostic`,
`ResourceUsageDiagnostic`, and `ObjectiveScoreDiagnostic`. Their complete field documentation is
...
JSON rows include `reserve_room_during_lab` so consumers can distinguish lecture-room occupancy from the separate
lab assignment. CSV uses standard quoting for commas, quotes, and newlines; absent room/lab assignments are empty
fields rather than the string `None`.

## Naming: two different `Course` concepts

- `scheduler.config.Course` is a string course-id type alias used by JSON configuration.
- `scheduler.models.Course` is the Z3-free runtime class containing credits and section identity.
- Generated values are `CourseInstance`; use `.course.course_id` for the base configuration id.
- `.lab` is the assigned lab name and `.lab_index` identifies its lab-marked meeting; both are `None` for no-lab
  sections.

## Full API surface

See **Reference** for Fern-generated pages covering importable modules, public callables, parameters, exceptions,
execution semantics, and datatype fields. `Scheduler`, `RoomConfig`, and `LabConfig` are public interfaces.
`SchedulingProblem`, `SolverEngine`, `DiagnosticEngine`, and `ScheduleAuditor` are internal architectural
components despite being importable from their implementation modules.
