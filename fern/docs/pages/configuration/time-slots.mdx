...
<Schema type="TimeSlotConfig-Input" />

Both numeric settings are positive integers. `max_time_gap` is used by same-faculty hard rules and the packing
objectives described under **Scheduling rules and objectives**. `times` supplies the weekday blocks; `classes`
supplies the enabled and disabled meeting patterns.
...
```json
{
  "credits": 4,
  "meetings": [
    { "day": "MON", "duration": 110, "lab": true, "delivery": "in_person" },
    { "day": "WED", "start_time": "10:00", "duration": 110, "lab": false, "delivery": "online" }
  ],
  "start_time": "09:00",
  "disabled": false
}
...
<Schema type="ClassPattern" />

<Schema type="Meeting" />

`credits` selects the course credit value, and `meetings` defines the non-empty set of distinct-weekday meetings.

Each meeting has required `day` and positive `duration`, plus optional `lab` (default `false`), `start_time`
(default `null`), and `delivery` (`in_person` by default, or `online`). A meeting-level start overrides the
pattern-level start for that meeting. Meetings without either fixed start use the day's block spacing. A lab-marked
meeting must be in person.

The meeting delivery values determine the pattern modality: all in-person is `in_person`, all online is `online`,
and a mixture is `hybrid`. A course can select only a pattern matching its configured `modality`.
