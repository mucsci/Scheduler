...
- The assigned pattern must fit entirely within the assigned faculty member's availability and every listed
  `alternate_faculty` member's availability.
...
| `pack_rooms` | One point for different-course pairs using the same non-null room when a pair of room-occupying physical meetings is adjacent. Online meetings and unreserved lab meetings do not count. |
