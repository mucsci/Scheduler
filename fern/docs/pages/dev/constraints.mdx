...
- Create and own Z3 variables exclusively through `CourseVariables` inside solver artifacts. Solver-owned `Course`
  copies retain the same variables as legacy compatibility mirrors, while `SchedulingProblem.courses` remains
  solver-independent. Each course section has no lab when its candidate list is empty, or exactly one required lab
  assignment when that list is non-empty.
