...
def generate_valid_start_times(
    day_time_blocks: List[TimeBlock],
    ...
    for time_block in day_time_blocks:
        ...
        while (
            current_minutes + meeting_duration <= end_minutes
        ):  # Ensure full meeting duration fits
...
def check_110_minute_alignment(
    ...
    has_110_minute = any(
        meeting.duration == 110 for idx, meeting, meeting_start_time in combination
    )
    ...
    cache_key = (
        tuple(sorted(config.times.keys())),
        tuple(
            sorted(
                meeting.duration
    ...
    if cache_key not in check_110_minute_alignment._cached_meeting_times:
        ...
        for day, time_blocks in config.times.items():
            for duration in all_durations:
                day_start_times = generate_valid_start_times(
                    time_blocks, duration, start_time
                )
        ...
        check_110_minute_alignment._cached_meeting_times[cache_key] = (
            all_meeting_times,
            all_meeting_end_times,
        )

    all_meeting_times, all_meeting_end_times = (
        check_110_minute_alignment._cached_meeting_times[cache_key]
    )
    ...
    for idx, meeting, meeting_start_time in combination:
        if meeting.duration == 110:
            ...
            start_aligned = any(
                abs(start_minutes - time_to_minutes(*parse_time(t))) <= max_diff
                for t in all_meeting_times
            )
            end_aligned = any(
                abs(end_minutes - time_to_minutes(*parse_time(t))) <= max_diff
...
def generate_time_combinations(
    meetings: List[Meeting], config: TimeSlotConfig, start_time: Optional[str] = None
) -> List[List[Dict]]:
    ...
    for i, meeting in enumerate(meetings):
        ...
        if day in config.times:
            valid_times = generate_valid_start_times(
                config.times[day], meeting.duration, start_time
            )
...
class TimeSlotConfiguration:
    ...
    def time_slots(self, credits: int) -> list[TimeSlot]:
        ...
        if credits in self._cache:
            return self._cache[credits]

        generated_slots = []

        # Find all class patterns for the requested credits
        for class_pattern in self.config.classes:
            ...
            if class_pattern.credits == credits:
                ...
                time_combinations = generate_time_combinations(
                    meetings, self.config, class_pattern.start_time
                )
        ...
        self._cache[credits] = generated_slots
